...
class WodRoll:
    ...
    def roll_dice(self):
        ...
        return self.nrolls, self.difficulty, rolls, message
    ...
    def get_message(self, rolls):
        ...
        if result == 0:
            return "Ningún éxito. ({})".format(result)
        elif result == -1:
            return "Fallo crítico. (1 falla)"
        elif result < -1:
            return "Que tirada tan mala. ({} fallas!)".format(result*(-1))
        elif result == 1:
            return "Usted tuvo 1 éxito"
        elif result > 1:
            return "Usted obtuvo {} exitos!".format(result)

    def count_success(self, seq):
        """Returns the amount of successes in a sequence of roll """
        return sum(1 for item in seq if item >= self.difficulty)
